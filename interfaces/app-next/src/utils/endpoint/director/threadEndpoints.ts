...
export const threadsEndpoint = createEndpointsNodes({
  messages: {
    getAll: ({ id }: { id: ThreadId }) =>
      queryOptions({
        ...
        queryFn: () =>
          get(`/threads/${id}/messages`, 'director').then((data) =>
  ...
  getAll: () =>
    queryOptions({
      ...
      queryFn: () =>
        get('/threads', 'director').then((data) => {
  ...
  get: ({ id }: { id: ThreadId }) =>
    queryOptions({
      queryKey: ['threads', id],
      queryFn: () =>
        get(`/threads/${id}`, 'director').then((data) => {
          return safeParseWithLog(threadsWithUserSchema, data);
        }),
    }),
  post: () =>
    mutationOptions({
      mutationFn: (data: NewThread) => post<Thread, NewThread>('/threads', data, 'director'),
      onSuccess: () => {
