    ...
    return match(query)
        ...
        .with({ status: "success" }, ({ data: currentRate }) => {
            if (!currentRate) {
                return (
                    <NoRateState
                        onNavigate={() => handleNavigate("/admin/savings-rates")}
                        t={t}
                    />
                );
            }
            ...
            return (
                <ActiveRateState
                    ...
                    onNavigate={() => handleNavigate("/director/savings-rate/new")}
                    t={t}
                />
            );
        })
...
const NoRateState = memo(({ onNavigate, t }: {
    onNavigate: () => void;
    ...
    <div className="relative overflow-hidden rounded-2xl bg-linear-to-br from-amber-500 via-orange-600 to-red-700 p-8 md:p-12">
        <div className="relative text-white text-center space-y-4">
            ...
            <p className="text-white/80">{t("noEffectText")}</p>
            <Button
                size="lg"
...
NoRateState.displayName = 'NoRateState';



const ActiveRateState = memo(({ currentRate, onNavigate, t }: {
    currentRate: SavingRate;
    onNavigate: () => void;
    t: ReturnType<typeof useTranslations>;
}) => (
    <div className="relative overflow-hidden rounded-2xl bg-linear-to-br from-rose-700 via-red-600 to-pink-900 mb-10">
        <div className="absolute inset-0">
            <div className="absolute inset-0 bg-[url('/grid.svg')] opacity-10" />
            <div className="absolute -right-32 -top-32 h-96 w-96 rounded-full bg-yellow-400/20 blur-3xl animate-pulse" />
            <div className="absolute -left-32 -bottom-32 h-96 w-96 rounded-full bg-blue-400/20 blur-3xl animate-pulse" style={{ animationDelay: '1s' }} />
        </div>
...
ActiveRateState.displayName = 'ActiveRateState';

const ImpactCard = memo(({
    label,
    amount,
    rate,
    highlighted = false,
    ...
    const { yearlyGain, monthlyGain, dailyGain } = useMemo(() => {
        const yearlyGain = (amount * rate) / 100;
        return {
            yearlyGain,
            monthlyGain: yearlyGain / 12,
            dailyGain: yearlyGain / 365
        };
    }, [amount, rate]);
