...
export class OrdersController {
    ...
    static async getAllByActionStatusAndUser(req: AuthRequest, res: Response, next: NextFunction){
        try {
            const userId = req.user?.userId;
            if (!userId) return res.status(401).json({ message: "Unauthorized" });
            ...
            const result = await orderFactory().getAllByActionAndStatusAndUserId.execute({
                userId,
                actionId: ISIN,
                status
            });
    ...
    static async getPortfolioByISIN(req: AuthRequest, res: Response, next: NextFunction){
        try {
            const userId = req.user?.userId;
            if (!userId) return res.status(401).json({ message: "Unauthorized" });
    ...
    static async placeOrder(req: AuthRequest, res: Response, next: NextFunction){
        try {
            ...
            const { IBAN, quantity, price } = orderSchema.pick({
                IBAN: true,
                quantity: true,
                price: true,
            })
