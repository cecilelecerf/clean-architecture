...
export class TransactionMapper {
    static mapRowToTransaction(row: RowDataPacket, prefix: string = ""): TransactionEntity {
        const fromAccountId = IBAN.from(row[`${prefix}from_account_id`]);

        const toAccountId = IBAN.from(row[`${prefix}to_account_id`]);

        const amount = Money.from({ amount: row[`${prefix}amount`], currency: row[`${prefix}currency`] });
