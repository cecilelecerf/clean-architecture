...
export class FormuleMapper {
    static mapRowToFormule(
        ...
        const interestRate = Percentage.from({ value: row[`${prefix}interest_rate`] });
        const insuranceRate = Percentage.from({ value: row[`${prefix}insurance_rate`] });
        
        const minAmount = row[`${prefix}min_amount`] != null && row[`${prefix}currency`]
            ? Money.from({ amount: row[`${prefix}min_amount`], currency: row[`${prefix}currency`] })
            : undefined;
        
        const maxAmount = row[`${prefix}max_amount`] != null && row[`${prefix}currency`]
            ? Money.from({ amount: row[`${prefix}max_amount`], currency: row[`${prefix}currency`] })
            : undefined;
        ...
        return FormuleCreditEntity.from({
            ...
            isActive: row[`${prefix}is_active`] === 1, // Permet de bien récupérer un bool et non un number
    ...
    static mapDocToFormule(doc: any): FormuleCreditEntity {
        ...
        const minAmount = doc.minAmount != null && doc.currency
          ? Money.from({ amount: doc.minAmount, currency: doc.currency })
          : undefined;

        const maxAmount = doc.maxAmount != null && doc.currency
