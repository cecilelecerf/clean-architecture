...
import { CreditRepositoryMongo } from "@infrastructure/adapters/db/mongo/repositories/CreditRepositoryMongo";
...
export async function generateCreditsMongo(
  ...
  for (const raw of rawCredits) {
    try {
      ...
      if (!client) {
        console.warn("Client manquant pour le crédit");
        continue;
      }
      ...
      // Crédit temporaire pour calcul de la mensualité
