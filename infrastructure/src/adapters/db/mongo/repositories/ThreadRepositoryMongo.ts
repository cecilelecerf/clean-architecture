...
export class ThreadRepositoryMongo implements ThreadRepository {
  ...
  async findAllWithUserByParticipantId(
    ...
    threadsDocs.forEach((thread) => {
      if (thread.administratorId)
        userIdsSet.add(thread.administratorId.toString());
      thread.participantsId?.forEach((id: string) =>
    ...
    const threads: ThreadEntityWithUsers[] = threadsDocs.map((doc) => {
      const adminDoc = doc.administratorId
        ? usersMap.get(doc.administratorId.toString())
        : null;
      ...
      const mappedParticipants: (UserEntity | null)[] = (
        doc.participantsId || []
      ).map((pid: string): UserEntity | null => {
      ...
      const thread = ThreadEntity.from({
        ...
        administratorId: doc.administratorId,
        participantsId:
          doc.participantsId?.map((id: string) => id.toString()) || [],
        title: doc.title,
        createdAt: doc.createdAt,
        updatedAt: doc.updatedAt,
        isClose: doc.isClose,
  ...
  async findWithUserById(
    ...
    if (doc.participantsId?.length)
      userIds.push(...doc.participantsId.map((id: string) => id.toString()));
  ...
  async findAllWithUserByAdministratorId(
    ...
    threadsDocs.forEach((thread) => {
      if (thread.administratorId)
        userIdsSet.add(thread.administratorId.toString());
      thread.participantsId?.forEach((pid: string) =>
        userIdsSet.add(pid.toString())
      );
    });
